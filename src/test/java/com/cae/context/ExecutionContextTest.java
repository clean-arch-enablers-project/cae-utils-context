...
class ExecutionContextTest {
    ...
    @Test
    @DisplayName("Should be able to add a resource to the bag")
    void shouldBeAbleToAddResourceToTheBag(){
        var context = ExecutionContext.ofNew();
        var myResource = new ArrayList<String>();
        context.putResource("some-resource-of-mine", ExecResource.of(myResource, ctx -> System.out.println("teste!")));
        Assertions.assertTrue(context.getResource("some-resource-of-mine", ArrayList.class).isPresent());
    }

    @Test
    @DisplayName("Should throw InternalMappedException when trying to put a resource already present in the bag")
    void shouldThrowInternalMappedExceptionWhenTryingToPutAResourceAlreadyPresentInTheBag(){
